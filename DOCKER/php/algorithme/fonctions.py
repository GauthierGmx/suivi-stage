...
def recup_donnees_etudiant(idEtud: int, cursor) -> List[List[Any]]:
    ...
    # Exécuter la requête
    cursor.execute(
        """SELECT etudiants.idUPPA, etudiants.nom, etudiants.prenom, entreprises.longitudeAdresse, entreprises.latitudeAdresse, entreprises.codePostal, entreprises.numSIRET FROM entreprises 
            JOIN fiche_descriptives ON entreprises.numSIRET=fiche_descriptives.numSIRET
...
def est_etudiant_deja_present_ville(codePostal, idUPPA, cursor) -> bool:
    ...
    cursor.execute(
        """
        SELECT entreprises.numSIRET, entreprises.raisonSociale, entreprises.codePostal, etudiants.nom, COUNT(etudiants.idUPPA) AS nbEtudiants
        FROM entreprises 
        JOIN fiche_descriptives ON entreprises.numSIRET = fiche_descriptives.numSIRET
        JOIN etudiants ON fiche_descriptives.idUPPA = etudiants.idUPPA
        WHERE LEFT(entreprises.codePostal, 2) NOT IN ('64', '40') AND entreprises.codePostal = %s AND etudiants.idUPPA != %s
        GROUP BY entreprises.numSIRET, entreprises.raisonSociale, entreprises.codePostal, etudiants.nom;
        """
    ,(codePostal, idUPPA)
    )
...
def est_deja_dans_entreprise(numSIRETEntreprise: int, idUPPA: int, cursor) -> bool:
    """
    Cherche dans la base si un étudiant est déjà présent dans cette entreprise
    
    Paramètre:
        - numSIRETEntreprise : l'identifiant de l'entreprise
    ...
    cursor.execute(
        """
        SELECT entreprises.numSIRET, entreprises.raisonSociale, entreprises.codePostal, etudiants.nom, etudiants.idUPPA
        FROM entreprises 
        JOIN fiche_descriptives ON entreprises.numSIRET = fiche_descriptives.numSIRET
        JOIN etudiants ON fiche_descriptives.idUPPA = etudiants.idUPPA
        WHERE entreprises.numSIRET = %s AND etudiants.idUPPA != %s;
        """,
        (numSIRETEntreprise,idUPPA)
    )
